...
def as_plain_text(passport: Passport) -> str:
    ...
    # Stated before the findings, not after them. What was checked has to be
    # read before what was found means anything.
    if passport.coverage is not None:
        lines += [
            f"COVERAGE      {passport.coverage.level.value}",
            "",
            "WHAT COULD BE ASSESSED",
            f"  {passport.coverage.message}",
        ]
        if passport.coverage.unassessed:
            lines += ["", "WHAT WAS NOT ASSESSED"]
            lines += [f"  - {item}" for item in passport.coverage.unassessed]
