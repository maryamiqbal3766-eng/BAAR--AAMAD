...
from modules.profile import product_terms, stems
...
def product_matches(declared: str, case_terms: set[str]) -> bool:
    """Does a source's declared product term describe these goods?

    The corpus declares what it covers — "leather articles", "any goods" — and
    the case supplies the exporter's own words. A declared term matches when
    EVERY significant word of it is present in the case's words.

    That direction matters. "leather bags" matching "handmade full-grain
    leather shoulder bags" is correct: the exporter described leather bags
    with extra detail. The reverse is not true — a case for "bags" must not
    pull in a source written for leather bags, because we do not know what
    these bags are made of. Requiring the declared term to be fully covered,
    rather than merely overlapping, is what keeps that asymmetry.
    """
    if declared.strip().lower() == ANY_PRODUCT:
        return True
    wanted = stems(declared)
    return bool(wanted) and wanted <= case_terms
...
def _applies_to_case(
    ...
    product_ok = not source.products or any(
        product_matches(term, product_scope) for term in source.products
    )
