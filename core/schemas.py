...
class InterpretationSource(str, Enum):
    """Who wrote the prose of a requirement.

    The distinction is shown to the user, because a reader is entitled to know
    which words came from a model and which were curated by a person against
    the source. It never affects authority: the CITATION is the authority
    either way, and the deterministic checks are the same in both cases.
    """

    #: The model read the retrieved passages and wrote this, and every field
    #: it wrote passed the grounding gate in modules.interpretation.
    AI_INTERPRETED = "AI_INTERPRETED"
    #: Human-curated wording from the corpus. Used when the model is
    #: unavailable, or when its output did not survive the grounding gate.
    CURATED = "CURATED"


class AgentDecision(str, Enum):
    """What the agent decided after seeing one observation."""
...
class StepOrigin(str, Enum):
    """Who chose a step in the agent loop.

    Both appear in a normal run. The model investigates; Python then runs
    whatever mandatory checks the model did not, so the agent can only ever
    ADD to the deterministic floor.
    """

    MODEL = "MODEL"
    MANDATORY_BACKFILL = "MANDATORY_BACKFILL"
...
class ShipmentInfo(_Base):
    buyer_name: str = ""
    buyer_address: str = ""
    #: The exporter's own reference for this consignment. Intake only — no
    #: check reads it.
    reference: str = ""
    invoice_number: str = ""
    invoice_date: str = ""
...
class CaseProfile(_Base):
    ...
    #: An HS code the exporter states for their own goods. Recorded as given
    #: and never treated as determined by BAAR-AAMAD — classification is a
    #: legal call we do not make. See classification_context below.
    hs_code: str = ""
...
class Interpretation(_Base):
    """Provenance for the PROSE of one requirement.

    Records whether a model wrote it, which retrieved passages it was allowed
    to write it from, and — when the model's output was rejected — why the
    curated wording is showing instead. Rendered in the AI Activity view, so a
    reader can follow:

        retrieved chunk -> AI interpretation -> structured requirement

    Never affects what is checked. `expected_document_types` and the
    deterministic checks behind a requirement are curated and are not offered
    to the model at all.
    """

    source: InterpretationSource = InterpretationSource.CURATED
    #: The model that produced it, exactly as the API reported it. Empty for
    #: curated wording.
    model: str = ""
    #: Evidence/chunk ids the model cited. Every one is verified to have been
    #: retrieved for this case before the interpretation is accepted.
    grounded_in: list[str] = Field(default_factory=list)
    #: Which fields the model authored. Anything not listed is curated.
    fields_from_ai: list[str] = Field(default_factory=list)
    #: Why the model's output was not used. Empty when it was.
    fallback_reason: str = ""
...
class Requirement(_Base):
    ...
    #: Where the prose above came from. Defaults to curated, which is what a
    #: requirement built without a model is.
...
class Passport(_Base):
    ...
    #: How much of this case the corpus could speak to, carried onto the
    #: Passport itself. A Passport that does not disclose partial coverage
    #: overstates the work that was done, so this travels with the document
    #: rather than living only on a screen the reader may not have seen.
...
# ---------------------------------------------------------------------------
#
# These two records are what the AI Activity view renders. Both are written
# from what actually happened, after it happened.
#
# WHAT THEY MUST NEVER CARRY: prompts, completions, and model reasoning. The
# gateway suppresses reasoning content at the transport (see core.llm), and
# nothing here is a place to reintroduce it. TS-2 is not negotiable and an
# observability surface is exactly where it would otherwise leak.


class LLMCallRecord(_Base):
    """One call to the model, as it will be shown on screen."""
    ...
    attempts: int = 1
    #: A short failure description when something went wrong. Never a prompt.
    detail: str = ""
...
class AgentStep(_Base):
    """One turn of the bounded agent loop: a tool, its result, what followed.

    `rationale` is the model's own ONE-LINE statement of what it is looking
    for. It is an intent, not a thought process — the gateway never receives
    reasoning content to begin with.
    """
    ...
    observation: str = ""
    decision: AgentDecision = AgentDecision.STOP
    rationale: str = ""
    chosen_by: StepOrigin = StepOrigin.MODEL
...
class ExportCase(_Base):
    ...
    #: What the AI actually did on the most recent run. Rebuilt each run like
    #: every other stage output, so a recheck shows the recheck's activity
